...
; (define (is-output-one-xyz789 output)
;     (string=? output "1"))
(newline)
; (newline)
; (newline)
; (display (string=? (n-read (fact zero)) "2"))

; (set! results (cons (string=? (n-read (fact zero)) "2") results))
(define factorialTests '())
(define (update-results written expr str)
    (set! factorialTests (cons (list (s-a (s-a written " -EXPECTED: ") str) (s-a "-ACTUAL: " expr) (string=? expr str)) factorialTests)))

(update-results "fact(zero)" (n-read (fact zero)) "2")
(update-results "fact(one)" (n-read (fact one)) "1")
(update-results "fact(two)" (n-read (fact two)) "2")
(update-results "fact(three)" (n-read (fact three)) "6")

(display "Results: ")
(define (filterForFails testList)
    (filter (λ (x) (eq? (last x) #f)) testList))

; (display (filter (λ (x) (eq? (last x) #f)) results))
