...
- **Persistent Binary Trees** — handler-encoded trees (`t-empty` / `t-node`, discriminated purely by application) with all four traversals (preorder, inorder, postorder, breadth-first), size/height/fold/map/mirror, and comparator-parameterized binary-search-tree insert, lookup, min, max, depth, and delete. Insertion and deletion share untouched subtrees, so every older tree stays usable (`trees.rkt`)
- **Persistent Queues** — the classic two-list FIFO queue: elements arrive on the back list and leave from the front, with the back reversed into a fresh front when it runs dry (`queues.rkt`)
- **Persistent Leftist Heaps** — rank-carrying min-heaps where merge is the one core operation: insert merges a singleton, delete-min merges the children (`heaps.rkt`)
- **Comparator-Parameterized Sorting** — merge sort, quicksort (via `_filter`), and heapsort, each taking the ordering as an argument so one implementation sorts Church naturals, integers, rationals, and binary numbers alike (`sorting.rkt`)
- **Graphs and Traversals** — adjacency-list graphs parameterized by a vertex-equality predicate, with DFS, BFS, reachability, path finding, unweighted shortest distances, directed cycle detection, connected components, and topological sort — every traversal threading its visited set through the recursion as an explicit lambda list, no mutation anywhere (`graphs.rkt`)
- **More Binary-Number Algorithms** — shifts, width-relative NOT, bit length, popcount, bitwise AND/OR/XOR, binary↔Church conversions computed entirely by lambda terms, integer square root, modular exponentiation, the extended Euclidean algorithm with modular inverses, trial-division and Miller–Rabin primality, and Fibonacci by fast doubling (`binary-algorithms.rkt`)
